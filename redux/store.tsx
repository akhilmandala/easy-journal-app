  ...
  reducers: {
    addEntry(
      ...
      return {
        ...
        entryOrder: [...state.entryOrder, entry.id],
      };
    },
    removeEntry(
      state,
...
export const selectRecentCheckInsWithinRange = (range) => (state) => {
  ...
  let recentCheckIns = recentCheckInIds.map(({id}) => checkIns[id]);
...
export const selectAllCheckInAfterDate = (date) => (state) => {
  let {checkInOrder} = state.journalEntries
  if(checkInOrder.length > 1) {
    let filteredCheckIns = checkInOrder.reverse()
    let mostRecentEntry = filteredCheckIns.findIndex(({checkInDate}) => date < date )
